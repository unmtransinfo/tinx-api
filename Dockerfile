...
CMD sh -c "python manage.py migrate --database=tcrd_meta && python manage.py migrate --database=tcrd && python tinxapi/metadata.py && python manage.py build_solr_schema && python manage.py makemigrations && python manage.py runserver 0.0.0.0:8000"